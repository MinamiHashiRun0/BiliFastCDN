...
不想等它赢下排名、想直接优先用它，就把 `preferHk` 打开。
...
打开它（默认关），`auto` 就不取全场第一，而是取排名里**第一个香港节点**（`cn-hk-eq-*` 里实测最快的那个）。
它只改"从排名里挑谁"，不改测速本身 —— 候选池照旧全测、排名照旧按实测排，所以面板上仍然有全场最快的对比。
两种例外都会自动回到原行为：这一轮香港节点全被否掉（压根不在排名里）；某个分类填了固定主机名。
两个模块都认这个参数，响应侧与请求侧用的是同一条规则。
...
| `test/verify.html` | 离线验证套件，139 项断言，用浏览器跑 |
...
打开 `preferHk` 后第一行换成**实际会被用作目标的那一个**，并补一行全场最快作对比：

```
港优先 cn-hk-eq-01-01 · 12.0 Mbps
测速 3 分钟前 · 16 个节点
全场最快 tf-all-hw
改写 命中12 改写9
```
...
- 开了 `preferHk` 却是「港优先：本轮无香港节点通过测速（xxx 是全场最快）」= 香港那 8 个节点这轮全被否掉了，
  当前走的是全场最快，不用去翻日志。
...
- **`preferHk` 是"优先"不是"最快"**：香港节点即使慢于全场第一也会被选中，卡片第一行给的是它自己的
  实测速率、下一行才是全场最快，两者的差距就是为"离得近"付的价。另外某个节点也可能只服务某一类签名
  的地址（测速里被否掉的不会进排名），所以刚打开时先看一两轮卡片再决定要不要长期留着。
...
输出 `ALL n CHECKS PASSED` 或失败明细。139 项断言覆盖：

- URL 原语、参数解析（含 `xy_usource` 的百分号解码）、粘贴整条 URL 时只取主机名
- `auto`：有排名时收敛到第一、无排名时回落兜底主机、固定主机名压过排名、`hostPcdn` 固定时大陆镜像放过
- `preferHk`：两个后端都改取香港节点、固定主机名仍然压过它、香港节点全被否掉时退回全场第一、
  `prefer_hk` 别名、面板第一行换成实际目标并保留"全场最快"对比、两个模块的三条/六条脚本条目都带这个参数
